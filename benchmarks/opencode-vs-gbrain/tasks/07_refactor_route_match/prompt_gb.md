...
- **get_backlinks** `code/routing` — find ALL files that depend on Route.matches() before refactoring
- **traverse_graph** `code/routing` --depth 2 — understand which classes inherit from BaseRoute
- **search** `class Route` — find Route, WebSocketRoute, Mount, Host — all BaseRoute subclasses
- **get_page** `code/convertors` — understand param_convertors interaction with matches()

Use get_backlinks first to identify all callers that depend on matches() — you must not break any of them.
